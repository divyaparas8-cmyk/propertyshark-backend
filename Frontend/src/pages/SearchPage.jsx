...
export const SearchPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FC] text-[#111827] flex flex-col font-sans pt-16 sm:pt-20">
      ...
      <div className="bg-[#0A1020] border-b border-white/10 py-6 sm:py-8 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-xl sm:text-3xl font-black text-white mb-3 sm:mb-4">
            Search NYC Properties
          </h1>

          <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
            <div className="relative flex-1">
              <Search className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 absolute left-3.5 top-3 sm:top-3.5 pointer-events-none" />
              <input
                type="text"
                value={searchInput}
                onChange={(e) => setSearchInput(e.target.value)}
                placeholder="Search address, city, ZIP or BBL (e.g. 42-07 12th St)..."
                className="w-full pl-10 sm:pl-12 pr-4 py-2.5 sm:py-3.5 bg-[#10182D] border border-white/15 rounded-xl text-white placeholder-gray-400 font-medium text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-[#2563EB]"
              />
            </div>
            <button
              type="submit"
              className="px-6 py-2.5 sm:px-8 sm:py-3.5 bg-gradient-to-r from-[#2563EB] via-[#4F46E5] to-[#6D28D9] hover:opacity-95 text-white font-bold rounded-xl text-xs sm:text-sm transition-all shadow-lg shadow-indigo-500/25 cursor-pointer flex items-center justify-center gap-2"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 w-full flex-1">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 sm:mb-8 pb-4 border-b border-[#E5E7EB]">
          <div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1">
            <span className="text-xs font-bold text-[#667085] uppercase tracking-wider flex items-center gap-1 mr-1 shrink-0">
        ...
        {!loading && filteredResults.length === 0 && (
          <div className="bg-white rounded-3xl p-8 sm:p-12 text-center border border-gray-200 shadow-sm max-w-xl mx-auto my-8">
          ...
          <div className="grid gap-4 sm:gap-6">
            {filteredResults.map((property) => {
              ...
              return (
                <div
                  key={property.bbl}
                  className="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-gray-200/90 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6"
                >
                  <div className="space-y-3 sm:space-y-4 flex-1">
                    {/* Top Row Badges */}
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs">
                      <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-md bg-[#0A1020] text-white font-bold tracking-wider uppercase">
                        {property.borough}
                      </span>
                      <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-md bg-indigo-50 text-[#4F46E5] font-bold border border-indigo-100 uppercase">
                        {property.propertyType}
                      </span>
                      <span className="font-mono text-[#667085] bg-gray-100 px-2 py-0.5 rounded text-[11px] sm:text-xs font-semibold">
                        BBL: {property.bbl}
                      </span>
                      <span className="font-mono text-[#667085] bg-gray-100 px-2 py-0.5 rounded text-[11px] sm:text-xs font-semibold">
                        BIN: {property.bin}
                    ...
                    <div>
                      ...
                      <p className="text-xs font-semibold text-[#667085] flex items-center gap-1 mt-1">
                    ...
                    {/* Property Meta Grid */}
<<<<<<<< HEAD:frotnend/src/pages/SearchPage.jsx
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 pt-3 border-t border-gray-100 text-xs">
                      <div>
                        <span className="text-[#667085] block font-medium uppercase text-[10px] sm:text-[11px]">Owner</span>
                        <span className="font-bold text-[#111827] truncate block">{property.owner}</span>
                      </div>
                      <div>
                        <span className="text-[#667085] block font-medium uppercase text-[10px] sm:text-[11px]">Lot / Building Area</span>
...
>>>>>>>> 6726612c438012abe80863779702721c7da748e6:Frontend/src/pages/SearchPage.jsx
                          {property.lotAreaSqFt.toLocaleString()} sq ft / {property.buildingAreaSqFt.toLocaleString()} sq ft
                        </span>
                      </div>
                      <div>
<<<<<<<< HEAD:frotnend/src/pages/SearchPage.jsx
                        <span className="text-[#667085] block font-medium uppercase text-[10px] sm:text-[11px]">Year Built / Zoning</span>
                        <span className="font-bold text-[#111827] block">
                          {property.yearBuilt} • <span className="text-[#2563EB]">{property.zoning}</span>
                        </span>
                      </div>
                      <div>
                        <span className="text-[#667085] block font-medium uppercase text-[10px] sm:text-[11px]">Commercial FAR</span>
...
>>>>>>>> 6726612c438012abe80863779702721c7da748e6:Frontend/src/pages/SearchPage.jsx
                  ...
                  <div className="flex flex-row lg:flex-col items-center lg:items-end justify-between lg:justify-center gap-2.5 pt-3 lg:pt-0 border-t lg:border-t-0 border-gray-100">
                    <button
                      onClick={() => toggleSaveProperty(property.bbl, property.address)}
                      className={`px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl border text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer ${
                        saved
                          ? 'bg-indigo-50 border-indigo-200 text-[#4F46E5]'
                          : 'bg-gray-50 border-gray-200 text-[#667085] hover:bg-gray-100'
                      }`}
                    >
                      <Bookmark className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${saved ? 'fill-[#4F46E5]' : ''}`} />
                      <span>{saved ? 'Saved' : 'Save'}</span>
                    </button>

                    <button
                      onClick={() => handleViewProperty(property.bbl)}
                      className="px-4 py-2.5 sm:px-6 sm:py-3.5 bg-gradient-to-r from-[#2563EB] via-[#4F46E5] to-[#6D28D9] hover:opacity-95 text-white font-bold rounded-xl text-xs uppercase tracking-wider shadow-lg shadow-indigo-500/25 flex items-center gap-1.5 transition-all transform active:scale-95 cursor-pointer whitespace-nowrap"
                    >
