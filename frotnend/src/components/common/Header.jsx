...
export const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-[#0A1020]/95 backdrop-blur-md border-b border-white/10 shadow-xl transition-all duration-300">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
        {/* LEFT: Brand Logo */}
        <Link to="/home" className="flex items-center gap-2 sm:gap-3 group shrink-0">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-[#2563EB] via-[#4F46E5] to-[#6D28D9] flex items-center justify-center shadow-lg shadow-indigo-500/30 group-hover:scale-105 transition-transform shrink-0">
            <Building2 className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
          </div>
          <div>
            <span className="font-black text-xs xs:text-sm sm:text-xl tracking-wider uppercase text-white flex items-center gap-1 sm:gap-1.5 whitespace-nowrap">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider text-white bg-gradient-to-r from-[#2563EB] via-[#4F46E5] to-[#7C3AED] hover:opacity-95 shadow-lg shadow-indigo-500/30 flex items-center gap-2 transition-all transform active:scale-95 cursor-pointer whitespace-nowrap"
          >
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#0A1020] border-b border-white/10 px-4 pt-3 pb-6 space-y-3 animate-in fade-in slide-in-from-top duration-200">
          {navLinks.map((link) => {
            const isActive = location.pathname === link.path || (link.path === '/saved-properties' && location.pathname === '/saved');
            return (
              <Link
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-3 rounded-xl text-center font-black text-xs uppercase tracking-wider text-white bg-gradient-to-r from-[#2563EB] via-[#4F46E5] to-[#7C3AED] block shadow-md"
            >
              SEARCH PROPERTY
            </button>
          </div>
          {user && (
            <div className="pt-4 border-t border-white/10 flex items-center justify-between px-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-white">{user.name || 'Analyst'}</span>
              </div>
              <button
                onClick={() => {
                  setMobileMenuOpen(false);
                ...
                className="px-3 py-1.5 rounded-lg bg-white/10 text-xs font-semibold text-gray-200 hover:text-white"
              >
                Logout
              </button>
            </div>
